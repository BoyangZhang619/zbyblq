...
for( i of Object.keys(alphabet)) {
    document.querySelector("#fontList").innerHTML += `<li class="font"><p>${i}</p><p>${alphabet[i]}</p></li>`;
    console.log(alphabet[i].length);
    
}
document.querySelector(".font").addEventListener("click",(e) => {
    calculating(e.currentTarget);
})

function calculating(target) {
    let _key = target.children[0].innerText;
    let _value = alphabet[_key];
    let _inputText = document.querySelector("#inputText").value;
    let _outputText = "";
}